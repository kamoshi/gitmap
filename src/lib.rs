...
pub struct Options {
    ...
    pub revision: String,
    // In Go the getGitCommandFunc was customizable.
    // For simplicity, we omit that in this version.
}
...
fn to_git_info(entry: &str) -> Result<GitInfo> {
    // Split the entry using \x1f as delimiter.
...
pub fn map(opts: Options) -> Result<GitRepo> {
    ...
    let abs_repo_path = repo_path.to_string_lossy().to_string();
...
// Implement Clone for GitInfo so we can reuse the value when inserting into the map.
impl Clone for GitInfo {
    fn clone(&self) -> Self {
        GitInfo {
            hash: self.hash.clone(),
            abbreviated_hash: self.abbreviated_hash.clone(),
